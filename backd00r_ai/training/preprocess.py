...
def require_dependencies():
    ...
    return pd, SMOTE, train_test_split
...
def validate_input_frame(frame) -> None:
    required = {"label", "binary_target", *FEATURE_NAMES_23}
...
def split_70_15_15(frame, train_test_split):
...
def fit_minmax_bounds(train_frame) -> dict[str, dict[str, float]]:
    ...
    for feature in FEATURE_NAMES_23:
...
def apply_smote_to_training_fold(train_frame, SMOTE, logger: PreprocessLogger):
    ...
    X = positive_train.loc[:, FEATURE_NAMES_23].astype(float)
    ...
    for idx in range(len(X_resampled)):
        if idx < original_count:
            base = {
                ...
                if column not in FEATURE_NAMES_23
            }
        else:
            base = {
...
def ordered_columns(frame) -> list[str]:
    ...
    extras = [
        ...
        if column not in set(leading) and column not in set(FEATURE_NAMES_23)
    ]
    return [*leading, *extras, *FEATURE_NAMES_23]
...
def preprocess(
    ...
    pd, SMOTE, train_test_split = require_dependencies()
    logger = PreprocessLogger(log_path)
    ...
    train_frame, val_frame, test_frame = split_70_15_15(frame, train_test_split)
